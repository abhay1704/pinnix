...
export const searchQuery = (searchTerm) => {
  const query = `*[ _type == "pin" && (title match '${searchTerm}' || category match'${searchTerm}' || content match '${searchTerm}') ]{
        _id,
        title,
        "image": image.asset->url,
        content,
...
export const searchFeed = `*[ _type == "pin"] | order(_createdAt desc){ 
    _id,
    title,
    "image": image.asset->url,
    content,
...
export const getSimilarPins = (title, category) => {
  const query = `*[ _type == "pin" && (title match '${title}' ${
...
export const userPinsQuery = (id) => {
  const query = `*[ _type == "pin" && owner._ref == "${id}"][0...9]{
        _id,
        title,
        "image": image.asset->url,
        content,
